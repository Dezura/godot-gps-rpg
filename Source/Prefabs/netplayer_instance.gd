...
func _init(lat, lon, username, color, level) -> void:
...
func update_stats(lon, lat, level) -> void:
...
func _process(delta: float) -> void:
	if global_position != coords.game_position:
		...
		global_position = global_position.lerp(coords.game_position, min(1, 5 * delta))
	
	# Stop lerping if we are close enough
	if abs(global_position.x - coords.game_position.x) < 0.2:
		global_position.x = coords.game_position.x
	if abs(global_position.y - coords.game_position.y) < 0.2:
